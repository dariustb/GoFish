...
def pick_card(active_player:Player) -> Card:
	...
	picked_card = None
	card_face = input(active_player.name() + ', select a card from your hand (2-10, J, Q, K, or A): ').title()
...
def print_dialogue(active_player:Player, picked_player:Player,
	...
	print(active_player.name() + ': ' + picked_player.name() + ', do you have any ' + picked_card.face() + '\'s?')
